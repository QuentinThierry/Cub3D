...
static const int		g_inter_button_y
	= (WIN_Y - (WIN_Y / 10) * NB_OPTIONS_BUTTONS / 2)
	/ (NB_OPTIONS_BUTTONS * 2);
		...
		t_image *button_image, t_image *button_hovered_image)
{
	...
	while (i < NB_OPTIONS_BUTTONS)
	{
		...
		{
			opt_menu->buttons[i].pos = (t_vector2){g_button_pos_left_offset_x,
		...
		opt_menu->buttons[i].pos
			...
			* (i - NB_OPTIONS_BUTTONS / 2
				+ ((NB_OPTIONS_BUTTONS & 1) == 0))};
		i++;
	}
}
		...
		t_image *button_image, t_image *button_hovered_image)
{
	...
	opt_menu->exit_opt_button.size = g_exit_size;
	opt_menu->exit_opt_button.text = "";
	opt_menu->slider_fov.size
		= (t_vector2){g_slider_hor_size.x, g_slider_vert_size.y};
	opt_menu->slider_fov.pos = g_slider_fov_pos;
	game->menu->option_menu.slider_fov.percent = (float)
		(DFL_FOV - MIN_FOV) / (MAX_FOV - MIN_FOV);
	game->menu->option_menu.slider_fov.min_max_value
		= (t_vector2){MIN_FOV, MAX_FOV};
	game->menu->option_menu.slider_fov.linked_text
		= g_description_slider_button[0];
	opt_menu->slider_sound.size
		= (t_vector2){g_slider_hor_size.x, g_slider_vert_size.y};
	...
	game->menu->option_menu.slider_sound.linked_text
		= g_description_slider_button[1];
...
bool	init_pause_menu(t_game *game)
{
	...
	fill_pause_menu_button(&game->menu->pause_menu,
		button_image, button_hovered_image);
	return (true);
}
