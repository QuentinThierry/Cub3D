...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/10/15 17:20:54 by jvigny            #+#    #+#             */
/*   Updated: 2023/10/15 17:33:41 by jvigny           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline t_dvector2	_collision_hit_ver_nw(t_game *game, t_map **map,
					float step, t_info_collison *inf)
{
	...
	if (inf->pos.x < inf->new_pos.x)
		return (check_interactive_object(game, map, inf->new_pos),
			inf->new_pos);
	if ((map[(int)(inf->comp.y + DIST_WALL)][(int)(inf->pos.x) - 1]
			.type & WALL) == WALL
		|| (map[(int)(inf->comp.y - DIST_WALL)][(int)(inf->pos.x) - 1]
			.type & WALL) == WALL)
	{
...
__attribute__((always_inline))
static inline t_dvector2	_collision_hit_hor_nw(t_game *game, t_map **map,
					float step, t_info_collison *inf)
{
	t_dvector2	tmp;
					...
					t_info_collison *inf)
{
	...
	inf->pos = (t_dvector2){(int)inf->fpos.x + DIST_WALL,
		(int)inf->fpos.y + DIST_WALL};
	if (inf->pos.x > inf->fpos.x)
		inf->pos.x -= 1;
	if (inf->pos.y > inf->fpos.y)
		inf->pos.y -= 1;
	inf->comp.x = inf->fpos.x + fabs(inf->fpos.y - inf->pos.y) * angle * -1;
	inf->comp.y = inf->fpos.y + fabs(inf->fpos.x - inf->pos.x) / angle * -1;
	while (true)
	{
		if (inf->pos.y <= inf->comp.y)
		{
			res = _collision_hit_ver_nw(game, map, 1 / angle * -1, inf);
			if (res.x != -1)
				return (res);
		}
		else
		{
			res = _collision_hit_hor_nw(game, map, angle * -1, inf);
			if (res.x != -1)
				return (res);
		}
	}
