...
/*   Updated: 2023/07/07 20:19:59 by jvigny           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
								...
								char **map, float angle, t_vector2 map_size)
{
	...
	comp.x = fabs(fpos.y - map_pos.y) * angle;
	comp.y = fabs(fpos.x - map_pos.x) / angle;
	step = (t_fvector2){angle, 1 / angle};
	while (true)
	{
		if (map_pos.y >= comp.y + fpos.y)
		{
			if (map_pos.x >= map_size.x || (int)(fpos.y + comp.y) >= map_size.y)
				return ((t_fvector2){-1, -1});
			if (map[(int)(fpos.y + comp.y)][map_pos.x] == '1')
				return ((t_fvector2){map_pos.x, fpos.y + comp.y});
			comp.y += step.y;
			map_pos.x += 1;
		}
		else 
		{
			if ((int)(comp.x + fpos.x) >= map_size.x || map_pos.y >= map_size.y)
				return ((t_fvector2){-1, -1});
			if (map[map_pos.y][(int)(comp.x + fpos.x)] == '1')
				return ((t_fvector2){comp.x + fpos.x, map_pos.y});
			comp.x += step.x;
			map_pos.y += 1;
		}
	}
								...
								char **map, float angle, t_vector2 map_size)
{
	...
	comp.x = fabs(fpos.y - map_pos.y) * angle;
	comp.y = fabs(fpos.x - map_pos.x) / angle * -1;
	step = (t_fvector2){angle, 1 / angle * -1};
	while (true)
	{
		if (map_pos.y <= comp.y + fpos.y)
		{
			if (map_pos.x >= map_size.x || (int)(fpos.y + comp.y) < 0)
				return ((t_fvector2){-1, -1});
			if (map[(int)(comp.y + fpos.y)][map_pos.x] == '1')
				return ((t_fvector2){map_pos.x, comp.y + fpos.y});
			comp.y += step.y;
			map_pos.x += 1;
		}
		else
		{
			if ((int)(comp.x + fpos.x) >= map_size.x || map_pos.y < 0)
				return ((t_fvector2){-1, -1});
			if (map[map_pos.y - 1][((int)(comp.x + fpos.x))] == '1')
				return ((t_fvector2){comp.x + fpos.x, map_pos.y});
			comp.x += step.x;
			map_pos.y += -1;
		}
								...
								char **map, float angle, t_vector2 map_size)
{
	...
	comp.x = fabs(fpos.y - map_pos.y) * angle * -1;
	comp.y = fabs(fpos.x - map_pos.x) / angle;
	...
	while (true)
	{
		if (map_pos.y >= comp.y + fpos.y)
		{
			if (map_pos.x < 0 || (int)(fpos.y + comp.y) >= map_size.y)
				return ((t_fvector2){-1, -1});
			if (map[(int)(comp.y + fpos.y)][map_pos.x - 1] == '1')
				return ((t_fvector2){map_pos.x, comp.y + fpos.y});
			comp.y += step.y;
			map_pos.x += -1;
		}
		...
		{
			if ((int)(comp.x + fpos.x) < 0 || map_pos.y >= map_size.y)
				return ((t_fvector2){-1, -1});
			if (map[map_pos.y][((int)(comp.x + fpos.x))] == '1')
				return ((t_fvector2){comp.x + fpos.x, map_pos.y});
			comp.x += step.x;
								...
								char **map, double angle)
{
	...
	while (true)
	{
		if (map_pos.y <= comp.y + fpos.y)
		{
			if (map_pos.x < 0 || (int)(fpos.y + comp.y) < 0)
				return ((t_fvector2){-1, -1});
			if (map[(int)(comp.y + fpos.y)][map_pos.x - 1] == '1')
				return ((t_fvector2){map_pos.x, comp.y + fpos.y});
			comp.y += step.y;
		...
		{
			if ((int)(comp.x + fpos.x) < 0 || map_pos.y < 0)
				return ((t_fvector2){-1, -1});
			if (map[map_pos.y - 1][((int)(comp.x + fpos.x))] == '1')
				return ((t_fvector2){comp.x + fpos.x, map_pos.y});
			comp.x += step.x;
...
inline t_fvector2	get_wall_hit(t_fvector2 fpos, char **map, float angle, t_vector2 map_size)
{
	...
	// angle = (int)angle;
	// angle /= 100000;
	angle = fabs(tanf(angle * TO_RADIAN));
