...
/*   Updated: 2023/10/14 15:51:47 by jvigny           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_ray	_get_wall_hit_sw(t_dvector2 fpos,
								t_map **map, float angle, float player_angle)
{
	t_fvector2	step;
	t_dvector2	comp;
	t_dvector2	door;
...
t_ray	get_wall_hit(t_dvector2 fpos, t_map **map, float angle)
{
	...
	if (sign.x == 1 && sign.y == 1)
		...
		return (_get_wall_hit_sw(fpos, map, fabsf(tanf(angle * TO_RADIAN)),
				angle));
	else
		return (_get_wall_hit_nw(fpos, map, fabsf(tanf(angle * TO_RADIAN)),
				angle));
}
