...
/*   Updated: 2023/07/14 23:22:13 by jvigny           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_fvector2	get_wall_hit_2d(t_game *game, double angle)
{
	...
	while (true)
	{
		while ((sign.y == 1 && y >= comp.y - error) || (y <= comp.y + error && sign.y == -1))		//x
		{
			...
			if (game->map[((int)comp.y) / CHUNK_SIZE][x / CHUNK_SIZE + (sign.x == -1) * -1].symbol == '1')
		...
		while ((sign.x == 1 && x >= comp.x - error) || (x <= comp.x + error && sign.x == -1))		//y
		{
			...
			if (game->map[y / CHUNK_SIZE + (sign.y == -1) * -1][((int)comp.x) / CHUNK_SIZE].symbol == '1')
...
void	quadrillage(t_game *game)
{
	...
	while (y < game->map_size.y)
	{
		...
		{
			if (game->map[y][x / CHUNK_SIZE].symbol == '1'
				|| game->map[(y-1)][(x-1) / CHUNK_SIZE].symbol == '1')
				color = 0x404040;
			else
				color = 0x202020;
	...
	while (x < game->map_size.x)
	{
		...
		{
			if (game->map[y / CHUNK_SIZE][x].symbol == '1'
				|| game->map[(y-1) / CHUNK_SIZE][(x-1)].symbol == '1')
				color = 0x404040;
			else
				color = 0x202020;
