...
/*   Updated: 2023/10/15 17:34:18 by jvigny           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline t_dvector2	_collision_hit_ver_sw(t_game *game, t_map **map,
...
__attribute__((always_inline))
static inline t_dvector2	_collision_hit_hor_sw(t_game *game, t_map **map,
					float step, t_info_collison *inf)
{
	t_dvector2	tmp;
					...
					t_info_collison *inf)
{
	...
	while (true)
	{
		...
		{
			res = _collision_hit_ver_sw(game, map, 1 / angle * -1, inf);
			if (res.x != -1)
				return (res);
		}
		else
		{
			res = _collision_hit_hor_sw(game, map, angle * -1, inf);
			if (res.x != -1)
				return (res);
		}
	}
